...
```{r initialize, echo=TRUE, message=FALSE}
library(dplyr);
library(ggplot2);
library(data.table);
library(lattice);
...
```{r hist_raw_dailyTotalSteps, echo=TRUE, fig.height=4, fig.width=6}
...
```
**The MEAN total number of steps taken each days is `r steps_mean`.**
**The MEDIAN total number of steps taken each days is `r steps_median`.**
...
```{r plot_intervalAverage, echo=TRUE, fig.height=4, fig.width=6}
...
```
**The 5-minute interval that (on average across all days) contains the maximum number of steps is interval # `r max_interval`.**
...
```
**The numbers of rows with missing values is `r num_miss`.**
...
```{r imputeImplement, echo=TRUE, results="hide"}
...
```
**The MEAN total steps per day using imputed data is `r steps_imp_mean`.**
**The MEDIAN total steps per day using imputed data is `r steps_imp_median`.**

**The difference between imputed mean and raw mean total steps per day is `r mean_diff`.**

**The difference between imputed median and raw median total number of steps per day is `r median_diff`.**
...
```{r panelPlot, echo=TRUE, fig.height=6, fig.width=6}
